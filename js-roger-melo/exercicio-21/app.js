...
const oddOnly = randomNumbers.filter(getOddNumbers);
console.log(oddOnly);
...
const totalNumbers = crazyNumbers.reduce(countNumbersLessThan501, 0);
...
const gameNames = cart.reduce((acc, { name }) => `${acc}- ${name}\n`, "");
...
const before2000s = tarantinoMovies.filter(({ release }) => release < 2000);
console.log(before2000s);
...
const serieNames = tvShows.map(({ name }) => name);
